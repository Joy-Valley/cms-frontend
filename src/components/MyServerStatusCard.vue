...
import { ref } from 'vue'
import type { Data, ServerStatusResponse } from '@/api/server-status/type'

const serverStatusData = ref<Data>()

const cpuUsedPercent = ref(0)
const memUsedPercent = ref(0)
const sysDiskUsedPercent = ref(0)

serverStatusAPI.status().then((response: ServerStatusResponse) => {
  serverStatusData.value = response.data
  cpuUsedPercent.value = Math.round(100 - Number(response.data.cpu.free))
  memUsedPercent.value = Math.round(Number(response.data.mem.usage))
  sysDiskUsedPercent.value = Math.round(Number(response.data.dist[0].usage))
})
</script>

<template>
  <Card class="col-span-12 md:col-span-6 xl:col-span-3 h-[--dashboard-card-height]">
    <template #content>
      <div class="flex justify-between">
        <div>
          <p class="text-xl font-semibold mb-2">CPU</p>
          <div class="text-3xl font-medium">{{ serverStatusData?.cpu.cpuNum }}线程</div>
        </div>

        <div>
          <Knob v-model="cpuUsedPercent" valueTemplate="{value}%" readonly />
        </div>
      </div>
    </template>
  </Card>
  ...
  <Card class="col-span-12 md:col-span-6 xl:col-span-3 h-[--dashboard-card-height]">
    <template #content>
      <div class="flex justify-between">
        <div>
          <p class="text-xl font-semibold mb-2">内存</p>
          <div class="text-3xl font-medium">
            {{ serverStatusData?.mem.used }} / {{ serverStatusData?.mem.total }}GB
          </div>
        </div>
        <div>
          <Knob v-model="memUsedPercent" valueTemplate="{value}%" readonly />
        </div>
      </div>
    </template>
  </Card>
  ...
  <Card class="col-span-12 md:col-span-6 xl:col-span-3 h-[--dashboard-card-height]">
    <template #content>
      <div class="flex justify-between">
        <div>
          <p class="text-xl font-semibold mb-2">系统盘</p>
          <div class="text-3xl font-medium">
            {{ serverStatusData?.dist[0].used.split('GB')[0] }} /
            {{ serverStatusData?.dist[0].total }}
          </div>
        </div>
        <div>
          <Knob v-model="sysDiskUsedPercent" valueTemplate="{value}%" readonly />
        </div>
      </div>
    </template>
  </Card>
  ...
  <Card class="col-span-12 md:col-span-6 xl:col-span-3 h-[--dashboard-card-height]">
    <template #content>
      <div class="flex justify-between">
        <div>
          <p class="text-xl font-semibold mb-2">环境</p>
          <div class="text-3xl font-medium">
            {{ serverStatusData?.sys.osName }} / {{ serverStatusData?.sys.osArch }}
          </div>
        </div>
        <div v-if="serverStatusData?.sys.osName == 'linux'" class="w-24 h-24">
          <svg
